...
function steering_signal = create_steering_signal(maneuver_type, vehicle)
    ...
    switch lower(maneuver_type)
        case 'gaspar'
            ...
            % Criação do sinal usando lógica booleana exata do LQR
            delta = (t >= t1 & t < t2) .* (2 * sin(pi * (t - t1) / (t2 - t1))) + ...
                    (t >= t2 & t < t3) .* (-4 * sin(pi * (t - t2) / (t3 - t2))) + ...
                    (t >= t3 & t < tf) .* (2 * sin(pi * (t - t3) / (tf - t3)));
                    
            % **NÃO HÁ CONVERSÃO AQUI**. O código do LQR envia direto assim pro Simulink!
            
        case 'step'
            delta = [zeros(100,1); 2*ones(length(t)-100,1)]; % Step de 2
